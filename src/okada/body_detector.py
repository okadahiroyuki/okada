...
""" body_detector.py - Version 1.0 2015-10-11
...
import cv2
import cv2.cv as cv
from okada.ros2opencv2 import ROS2OpenCV2

class BodyDetector(ROS2OpenCV2):
    def __init__(self, node_name):
        super(BodyDetector, self).__init__(node_name)
    ...
    def detect_body(self, input_image):
        ...
        # The bodies variable holds a list of body boxes.
        # If one or more bodies are detected, return the first one.  
        if len(bodies) > 0:
            body_box = bodies[0]
        else:
        # If no bodies were detected, print the "LOST BODY" message on the screen
            if self.show_text:
                font_body = cv2.FONT_HERSHEY_SIMPLEX
                font_scale = 0.5
                cv2.putText(self.marker_image, "LOST BODY!", 
                            (int(self.frame_size[0] * 0.65), int(self.frame_size[1] * 0.9)), 
                            font_body, font_scale, cv.RGB(255, 50, 50))
            body_box = None
...
if __name__ == '__main__':
    try:
        node_name = "body_detector"
        BodyDetector(node_name)
        rospy.spin()
    except KeyboardInterrupt:
        print "Shutting down body detector node."
