...
""" body_tracker2.py - Version 1.1 2013-12-20

    Combines the OpenCV Haar body detector with Good Features to Track and Lucas-Kanade
...
from okada.body_detector import BodyDetector
from okada.lk_tracker import LKTracker

class BodyTracker(BodyDetector, LKTracker):
    def __init__(self, node_name):
        super(BodyTracker, self).__init__(node_name)
        
        self.n_bodies = rospy.get_param("~n_bodies", 1)
    ...
    def process_image(self, cv_image):
        try:
            ...
            # Step 1: Detect the body if we haven't already
            if self.detect_box is None:
                self.keypoints = list()
                self.track_box = None
                self.detect_box = self.detect_body(self.grey)
            else:
...
if __name__ == '__main__':
    try:
        node_name = "body_tracker"
        BodyTracker(node_name)
        rospy.spin()
    except KeyboardInterrupt:
        print "Shutting down body tracker node."
