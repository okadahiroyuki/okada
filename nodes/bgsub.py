...
#        self.depth_sub = rospy.Subscriber("/camera/depth_registered/image_raw", Image, self.depth_callback, queue_size=1)
    ...
    def image_callback(self, ros_image):
        ...
        sc=0
        for gw_y in range(0, img_size[0]/wsize_y):
            for gw_x in range(0, img_size[1]/wsize_x):
                for lw_y in range(0, wsize_y):
                    for lw_x in range(0, wsize_x):
                        fx = im_mask[gw_y*wsize_y+lw_y, gw_x*wsize_x+lw_x]
                        sc+=fx
                score.append(sc)
                sc=0
        print score
        print score.index(min(score)), score[score.index(min(score))]                    

        #一番動いた領域
        sci = score.index(min(score))
        xs = sci%(img_size[1]/wsize_x) *wsize_x 
        ys = sci%(img_size[0]/wsize_y) *wsize_y 
        xe = xs+wsize_x-1 
        ye = ys+wsize_y-1 
        
        cv2.rectangle(im_edge,(xs,ys),(xe,ye),(255,255,255),1)
